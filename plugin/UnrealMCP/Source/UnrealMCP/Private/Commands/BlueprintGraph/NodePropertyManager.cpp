	...
	const TSharedPtr<FJsonObject>& Params)
{
	...
	// === SELECT: Set value type (wildcard -> float/int/bool/string) ===
	if (Action.Equals(TEXT("set_value_type"), ESearchCase::IgnoreCase))
	{
		UK2Node_Select* SelectNode = Cast<UK2Node_Select>(Node);
		if (!SelectNode)
		{
			return CreateErrorResponse(TEXT("Node is not a Select node"));
		}
		...
		// Map type string to pin category
		FName PinCategory;
		...
		{
			return CreateErrorResponse(FString::Printf(TEXT("Unsupported type: %s. Use float, int, bool, or string."), *NewType));
		}

		// Get the return value pin and change its type
		UEdGraphPin* ReturnPin = SelectNode->GetReturnValuePin();
		if (!ReturnPin)
		{
			return CreateErrorResponse(TEXT("Select node has no ReturnValue pin"));
		}

		// Change the return pin type
		ReturnPin->PinType.PinCategory = PinCategory;
		ReturnPin->PinType.PinSubCategory = PinSubCategory;

		// Propagate type to all related pins (Option 0, Option 1, etc.)
	...
	// === GENERAL: Set default value on a pin ===
	if (Action.Equals(TEXT("set_pin_default"), ESearchCase::IgnoreCase))
	{
		...
		// Get default_value or property_value as string (accept string, number, or bool)
		FString DefaultValue;
		if (!Params->TryGetStringField(TEXT("default_value"), DefaultValue) &&
			!Params->TryGetStringField(TEXT("property_value"), DefaultValue))
		{
			double NumVal;
			if (Params->TryGetNumberField(TEXT("default_value"), NumVal) ||
				Params->TryGetNumberField(TEXT("property_value"), NumVal))
			{
			...
			else
			{
				...
				if (Params->TryGetBoolField(TEXT("default_value"), BoolVal) ||
					Params->TryGetBoolField(TEXT("property_value"), BoolVal))
				{
					DefaultValue = BoolVal ? TEXT("true") : TEXT("false");
				}
				else
				{
					return CreateErrorResponse(TEXT("Missing 'default_value' or 'property_value' parameter"));
				}
			}
		}
		...
		if (!Pin)
		{
			// Try finding by display name as fallback
		...
		// Use schema to set default value properly
