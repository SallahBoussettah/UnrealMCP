...
TSharedPtr<FJsonObject> FBPConnector::ConnectNodes(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Use the schema's TryCreateConnection which handles wildcard type
    // promotion, implicit casts, and breaking existing connections
    const UEdGraphSchema_K2* Schema = GetDefault<UEdGraphSchema_K2>();
    if (!Schema->TryCreateConnection(SourcePin, TargetPin))
    {
        // Fall back to manual compatibility check + MakeLinkTo
